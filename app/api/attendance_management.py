...
def get_college_lecture(db: Session, lecture_id: int, college_id: int):
    ...
    if lecture.status == "Cancelled":
        raise HTTPException(
            status_code=400,
            detail="This lecture has been cancelled. Attendance cannot be marked.",
        )
    return lecture
...
def get_lecture_attendance(
    ...
    students = (
        ...
        .order_by(Student.roll_no.asc(), Student.id.asc())
        .all()
    )
...
def mark_student_attendance(
    ...
    get_college_lecture(db, lecture_id, admin.college_id)
...
def mark_all_attendance(
    ...
    return {
        "success": True,
        "message": f"{updated} students marked {request.status}.",
