...
def _college_from_access_code(access_code: str, db: Session):
...
@router.get("/college/access-code/{access_code}/verify")
def verify_college_access_code(access_code: str, db: Session = Depends(get_db)):
    college = _college_from_access_code(access_code, db)
    return {"college_id": college.id, "college_slug": college.slug, "college_name": college.name, "verified": True}


@router.get("/college/access-code/{access_code}")
def resolve_college_by_access_code(access_code: str, db: Session = Depends(get_db)):
    college = _college_from_access_code(access_code, db)
    admin = (
        db.query(Admin)
...
def mark_attendance_public(college_slug: str, payload: dict, db: Session = Depends(get_db)):
    ...
    if isinstance(result, str):
        if result == "ALREADY_MARKED":
            ...
            return {"attendance_marked": False, "already_marked": True, "message": "Attendance already marked for this lecture.", "student_id": student.id, "roll_no": student.roll_no, "name": student.name, "lecture": _lecture_payload(lecture)}
        messages = {"NO_ACTIVE_LECTURE": "No active lecture for this student right now.", "LECTURE_NOT_FOUND": "Lecture not found.", "LECTURE_CANCELLED": "This lecture is cancelled.", "LECTURE_NOT_ACTIVE": "This lecture is not currently active.", "STUDENT_NOT_IN_LECTURE_CLASS": "Student is not enrolled in this lecture's class.", "STUDENT_NOT_FOUND": "Student not found."}
    ...
    return {"attendance_marked": True, "already_marked": False, "message": "Attendance marked successfully for this lecture.", "student_id": student.id, "roll_no": student.roll_no, "name": student.name, "lecture": _lecture_payload(result.lecture)}


@router.get("/{college_slug}/attendance/{roll_no}")
def get_public_attendance_report(college_slug: str, roll_no: str, db: Session = Depends(get_db)):
    student = db.query(Student).join(College).filter(College.slug == college_slug.strip().lower(), Student.roll_no == roll_no.strip()).first()
    if student is None:
        raise HTTPException(status_code=404, detail="Student not found")
    return {"student": {"roll_no": student.roll_no, "name": student.name, "department": student.department}, "records": get_student_attendance(db, student.id, student.college_id)}
