    ...
    if "students" in inspector.get_table_names():
        ...
        if "phone_no" not in columns:
            with engine.begin() as connection:
                connection.exec_driver_sql("ALTER TABLE students ADD COLUMN phone_no VARCHAR(30) NULL")
...
def ensure_admin_settings_columns():
    ...
    if "admins" in inspector.get_table_names():
        ...
        with engine.begin() as connection:
            if "notifications" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN notifications BOOLEAN NOT NULL DEFAULT 1")
            if "email_alerts" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN email_alerts BOOLEAN NOT NULL DEFAULT 0")
            if "sound_alerts" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN sound_alerts BOOLEAN NOT NULL DEFAULT 1")
            if "threshold" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN threshold INT NOT NULL DEFAULT 85")
            if "resolution" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN resolution VARCHAR(10) NOT NULL DEFAULT '1080p'")
            if "fps" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN fps VARCHAR(10) NOT NULL DEFAULT '30'")
            if "language" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN language VARCHAR(30) NOT NULL DEFAULT 'English'")
            if "name" not in columns: connection.exec_driver_sql("ALTER TABLE admins ADD COLUMN name VARCHAR(150) NULL")
...
def ensure_multitenancy_columns():
    ...
    with engine.begin() as connection:
        ...
        if legacy_id is None:
            result = connection.execute(text("INSERT INTO colleges (name, slug, is_active) VALUES (:name, :slug, 1)"), {"name": "Legacy College", "slug": "legacy-college"})
    ...
    if engine.dialect.name == "mysql":
        ...
        with engine.begin() as connection:
            for table, columns in (("admins", {"username", "email"}), ("students", {"roll_no", "email"})):
                for constraint in inspector.get_unique_constraints(table):
                    ...
                    if constrained in ({column} for column in columns) and re.fullmatch(r"[A-Za-z0-9_]+", constraint_name):
                        connection.exec_driver_sql(f"ALTER TABLE {table} DROP INDEX {constraint_name}")
            for table, index_name, fields in (("admins", "uq_admin_college_username", "college_id, username"), ("admins", "uq_admin_college_email", "college_id, email"), ("students", "uq_student_college_roll_no", "college_id, roll_no"), ("students", "uq_student_college_email", "college_id, email")):
                ...
                if index_name not in index_names: connection.exec_driver_sql(f"CREATE UNIQUE INDEX {index_name} ON {table} ({fields})")


def ensure_college_access_code_column():
    inspector = inspect(engine)
    if "colleges" not in inspector.get_table_names(): return
    columns = {column["name"] for column in inspector.get_columns("colleges")}
    if "access_code_hash" not in columns:
        with engine.begin() as connection: connection.exec_driver_sql("ALTER TABLE colleges ADD COLUMN access_code_hash VARCHAR(255) NULL")


def ensure_pending_college_registration_columns():
    inspector = inspect(engine)
    if "pending_college_registrations" not in inspector.get_table_names(): return
    columns = {column["name"] for column in inspector.get_columns("pending_college_registrations")}
...
def create_default_admin():
    with SessionLocal() as session:
        ...
        if college is None:
            ...
            session.add(college); session.commit(); session.refresh(college)
        exists = session.query(Admin).filter(Admin.username == "admin").first()
        if exists is None:
            create_admin(session, AdminCreate(username="admin", name="Administrator", email="admin@example.com", password="admin123"), college.id)
...
async def lifespan(app: FastAPI):
    ...
    scheduler = threading.Thread(target=absence_email_scheduler, args=(stop_event,), daemon=True, name="absence-email-scheduler")
    scheduler.start(); yield; stop_event.set(); scheduler.join(timeout=2)
...
app.include_router(student_router); app.include_router(recognition_router); app.include_router(report_router); app.include_router(auth_router); app.include_router(notification_router); app.include_router(public_router); app.include_router(lecture_router); app.include_router(lecture_schedule_router)
